document.addEventListener('DOMContentLoaded', function() {
    ...
    async function displayCartItems() {
        ...
        cartItemsContainer.innerHTML = '<p>Memuat keranjang...</p>';

        try {
            const items = await window.getCartContents();

            if (!items) {
                cartItemsContainer.innerHTML = '<p class="text-danger">Gagal memuat isi keranjang. Silakan coba lagi nanti.</p>';
                return;
            }
            ...
            cartItemsContainer.innerHTML = '<p class="text-danger">Tidak dapat menampilkan item keranjang.</p>';
        }
    }
    ...
    async function displayOrderSummary() {
        ...
        orderSummaryContainer.innerHTML = '<p>Menghitung ringkasan...</p>';

        try {
            const items = await window.getCartContents();
            if (!items) {
                 orderSummaryContainer.innerHTML = '<p class="text-danger">Tidak dapat menghitung ringkasan pesanan.</p>';
                return;
            }
            ...
            orderSummaryContainer.innerHTML = '<p class="text-danger">Tidak dapat menampilkan ringkasan pesanan.</p>';
        }
    }
    ...
    function showAddressFormView(currentAddress) {
        ...
        if(senderAddressSection) {
            ...
            document.getElementById('save-address-btn').addEventListener('click', async function() {
                ...
                if (!newAddressValue) {
                    showErrorNotification("Alamat tidak boleh kosong.");
                    return;
                }
                ...
                if (!userId || !token) {
                    showErrorNotification('Kesalahan autentikasi. Silakan masuk lagi.');
                    return;
                }
    ...
    if (payButton) {
        payButton.textContent = 'Lanjutkan ke Checkout';
        payButton.addEventListener('click', async function() {
            ...
            if (!userId || !token) {
                showInfoNotification('Silakan masuk untuk melanjutkan.');
                if (typeof window.showLoginForm === 'function') {
                    window.showLoginForm();
                }
                return;
            ...
            try {
              ...
              if (!response.ok) {
                  ...
                  throw new Error(`Gagal mengambil alamat. Status: ${response.status}. Details: ${errorDetails}`);
              }
              ...
              showErrorNotification(`Tidak dapat mengambil alamat pengguna: ${error.message}`);
            }
        });
    }
